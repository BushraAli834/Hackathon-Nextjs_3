...
// function SlugPage({ params }: { params: { slug: string } }) {
//   const products: Product[] = useSelector((state: any) => state.product);
...
//           <Image key={i} src={img} className="w-[70%]" alt={items.title} width={100} height={100} />
//         ))}
//       </div>

//       {/* Middle Main Image */}
//       <div className="lg:w-[500px] flex justify-center mt-4 sm:mt-0 order-1 sm:order-2">
//         <Image src={items.image[0]} className="w-[80%]" alt={items.title} width={100} height={100} />
//       </div>
...
//               className="w-[37px] h-[37px] rounded-full border"
...
//             <button
//               key={i}
//               onClick={() => setCartItem({ ...cartItem, size: [size] })}
//               className="w-[70px] h-[37px] rounded-[16px] bg-gray-500 text-white"
//             >
//               {size}
//             </button>
//           ))}
...
function SlugPage({ params }: SlugPageProps) {
  const products: Product[] = useSelector((state: any) => state.product);
  const items = products.find((item) => item.slug === params.slug);
